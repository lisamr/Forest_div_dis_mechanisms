...
#check out----------
fit_hurdletop5 <- read_rds('../../../Box/Stan_model_outputs/Big_Sur/fit_hurdletop5.RDS')

precis(fit_hurdletop5, depth =3, pars = c('B_bar', 'B','Bz_bar', 'Bz', 'sigma_p', 'sigma_pz', 'sd_j', 'va'), prob = .9)
...
ppc_dens_group <- function(i) {
  use <- datalist_hurdletop5$spID == i
  ppc_dens_overlay(datalist_hurdletop5$y[use], post_species$y_rep[1:50,use])
}
...
CI_BA <- t(apply(post_top5$B[,,3] + post_top5$B_bar[,3], 2, HPDI, .9))
crosses_zero_BA <- data.frame(linetypeBA = unname(ifelse(CI_BA[,1] * CI_BA[,2] > 0, 1, 2)), spID = 1:nrow(CI_BA)) 
CI_Occ <- t(apply(post_top5$Bz[,,3] + post_top5$Bz_bar[,3], 2, HPDI, .9))
...
Xsim2 <- expand_grid( #data for posterior predictions 
  ...
  slice(rep(row_number(), max(BA_top5$spID))) %>% 
  mutate(Richness = unzscore(Richness_z, BA_top5$Richness),
         spID = datalist_hurdletop5$spID_sim) %>% 
  left_join(crosses_zero_BA) %>% 
  left_join(crosses_zero_Occ) %>% 
  left_join(BA_top5 %>% distinct(Species, spID))
...
#Basal area
post_df <- f_PP(post_top5$mu_sim, Xsim2)
p1 <- ggplot(post_df, aes(Richness, mean, group = ForestAllianceType)) +
  ...
  geom_jitter(data = BA_top5 %>% filter(BA>0), aes(Richness, BA, color = as.factor(ForestAllianceType)), alpha = .7, width = .15, height = 0) +
  facet_grid(~Species)+
...
#occurence
post_df2 <- f_PP(post_top5$pz_sim, Xsim2)
...
ggsave('figures/hurdleGLMM_top5_unused.pdf', width = 6, height = 3)
